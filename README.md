...
def resize_image(image_path, new_height):
    ...
    return image
```
Function to change the size of an image file
<br>

```python
# 이미지의 배경을 검정색으로 바꾸는 함수
def change_background (image_path, new_color):
    ...
    return img
```
Function to change image background to desired color
<br>
...
```
Function to import pixel data from an image file
<br>
...
def save_pixel_data_4bit_color(pixel_data, width, height, file_path):
    ...
    file.close()
```
Function that stores pixel data in 4-bit color format
<br>

```python
# 함수 호출
image = resize_image("background.png", 240) # 이미지 파일 이름, 세로 픽셀 갯수 (가로 세로 비율 맞게 resizing)
...
```
Function that stores pixel data in an image in a text file in 4-bit color format
